...
// state: dung de luu trang thai
// parent: dung de luu nut cha cua mot nut moi duoc trien khai
// no_function: Luu stt phep toan da thuc hien tuong ung. Bai toan nay chung ta co 4 phep toan.
typedef struct Node{
	State state;
	struct Node* parent;
	int no_function;
...
// ham heuristic h1: so luong vi tr� sai kh�c cua trang th�i s1 so voi s2
// Xem giao trinh trang 71
...
/*
Node* find_node(State state, queue<Node*> q) {
	while(!q.empty()) {
		if(compareStates(q.front()->state, state))
			return q.front();
		q.pop();
...
// Su dung ham Heuristic trong tim kiem


//3.4.3. Giai thuat hau an (Greedy)
...
// Tham khao trang 68 trong giao trinh
...
Node* Astart(State init_state, State goal_state, int func){
	...
	// Khai bao hai hang doi Open va Close
	// Khac voi BFS, trong giai thuat Greedy cung nhu Best First Search,
	// chung ta su dung Hang Doi Uu Tien (xem giao trinh trang 59) (sap xep thu tu dua tren gia tri f cua cac node)
	priority_queue<Node*, vector<Node*>, node_cmp> priority_queue_Open; 	// Open la duong bien (Frontier) trong giao trinh
	priority_queue<Node*, vector<Node*>, node_cmp> priority_queue_Close;	// Close la Explored trong giao trinh
	int step = 0;
    ...
    while (!priority_queue_Open.empty()) {
        ...
        //Ap dung cac hanh dong de tao ra cac trang thai moi
        for (int option = 1; option <= MAX_OP; option++) {
        	//printf("\nAction %s\n", action[op]);
            State new_state;
            if (call_operator(node->state, new_state, option)) {
				if(find_state_in_priority_queue(new_state, priority_queue_Close))
				{
					//printf("State found in Close!");
                    continue;
                } 
                
                Node* n = find_node(new_state, priority_queue_Open);
                if (n == NULL) {
                    n = new Node();
                    n->parent = node;
                    n->state = new_state;
